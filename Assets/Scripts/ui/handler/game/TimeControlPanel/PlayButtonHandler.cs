...
public class PlayButtonHandler : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
    ...
    void Update() {
        if (hovering) {
            myText.color = hover;
        } else if (TimeController.instance.isNormalSpeed() && TimeController.instance.isPlaying()) {
